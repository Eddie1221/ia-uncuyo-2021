...
- ![](RackMultipart20230730-1-kkrz0p_html_f6aa33983473d1ef.png)es la probabilidad de una acción **a** en el estado **s** en el tiempo **t** conducirá al estado **s'** en el tiempo **t+1**
- ![](RackMultipart20230730-1-kkrz0p_html_25a5ffbd99a63c7e.png)Es la recompensa inmediata decidida al transicionar del estado **s** al estado **s´**
...
_ **Deep learning y Redes Neuronales** _

El _Deep learning_ es una rama del _Machine learning_ en la cual se emplean técnicas para que las máquinas aprendan por sí solas, se puede considerar como una forma de automatizar el análisis predictivo. El deep learning funciona mediante el análisis constante de la información que recibe. Para lograrlo, hace uso de redes neuronales artificiales, con el objetivo de que se obtenga un proceso de aprendizaje más avanzado.
...
![](RackMultipart20230730-1-kkrz0p_html_61b869a49c8baedb.png)

La cual se expresa matemáticamente con la siguiente fórmula: , siendo:

- red = w1\*x1 + … + wn\*xn - la suma ponderada.
...
![](RackMultipart20230730-1-kkrz0p_html_ce1c89b0d5d36fb0.png)
...
_Q-learning Deep Q-learning_

![](RackMultipart20230730-1-kkrz0p_html_c4352173b9028bcb.png) ![](RackMultipart20230730-1-kkrz0p_html_f4f50296d9fa8d16.png) Retrieved from: https://www.geeksforgeeks.org/deep-q-learning/
...
![](RackMultipart20230730-1-kkrz0p_html_aeec570d3b1edbf7.png)

Retrieved from: [https://es.wikipedia.org/wiki/Archivo:Extrema\_example\_es.svg](https://es.wikipedia.org/wiki/Archivo:Extrema_example_es.svg)
...
Se implementó el archivo _Qtable.py_ que permite inicializar los valores de la _Q-table_ que utilizara el agente, en este se definen los estados que tendrá la tabla y las acciones disponibles para el agente. Los estados están definidos por 8 variables:
...
El agente de _Q-learning_ está implementado en el archivo _agent\_QL.py._ Para aprender el agente juega al snake cuantas veces indique el número máximo de episodios utilizando la _Q-table_ para almacenar el aprendizaje obtenido, reiniciando el juego cada vez que la serpiente muere o llega a 1000 pasos sin comer ninguna fruta. Antes de realizar cada acción en el juego, el agente recopila información del entorno para construir el estado en el que se encuentra y busca los _Q-values_ de dicho estado en la tabla, luego realiza ya sea una acción aleatoria o la acción que corresponde al mayor _Q-value_ de dicho estado, por último actualiza dicho _Q-value_ en base a la recompensa por la acción realizada utilizando la ecuación de Bellman.

_ **Implementación Deep Q-learning** _

El agente de Deep Q-learning está implementado en el archivo _agent\_DQL.py_ y se basa principalmente en las funcionalidades que añade la librería de python: _keras_, la cual es una biblioteca de redes neuronales que está especialmente diseñada para posibilitar la experimentación en poco tiempo con redes de aprendizaje profundo.
...
Al ser variables binarias los posibles valores de cada una de ellas son el 1 y 0. Esto deja un total de , aunque hay muchos estados a los que el agente nunca llegará debido a que hay variables que nunca tendrá valor de 1 en el mismo estado (como las variables que corresponden a la dirección de la serpiente, la cual nunca mirará a la izquierda y arriba al mismo tiempo, por ejemplo).
...
![](RackMultipart20230730-1-kkrz0p_html_f254e39d1dca23c8.png) ![](RackMultipart20230730-1-kkrz0p_html_79dca16a7bcbce31.png) ![](RackMultipart20230730-1-kkrz0p_html_2f22569bcdf07b60.png)

20x20 30x30 50x50

_ **Entorno sin Obstáculos vs Entorno con Obstáculos:** _
...
![](RackMultipart20230730-1-kkrz0p_html_a2a363d2e1e33fbb.png) ![](RackMultipart20230730-1-kkrz0p_html_bf77445230d2f963.png) ![](RackMultipart20230730-1-kkrz0p_html_baecef57043de0de.png)

1% de probabilidad 2% de probabilidad 4% de probabilidad

_ **Agente de control aleatorio vs Q-learning:** _
...
_ **Q-learning vs Deep Q-learning:** _
...
_ **Métricas de los resultados a analizar:** _
...
_ **Ajuste de Parámetros: Entrenamiento sin obstáculos** _
...
- **Probabilidad de exploración:** 1.0. Al comienzo todos los _Q-values_ están inicializados en 0 por lo que se eligen las acciones de manera aleatoria.
...
_ **Ajuste de Parámetros: Entrenamiento con obstáculos** _
...
_ **Resultado del aprendizaje respecto al tamaño del problema:** _
...
![](RackMultipart20230730-1-kkrz0p_html_97dd459546d956e2.png)
...
![](RackMultipart20230730-1-kkrz0p_html_ecd573c00364a597.png)
...
![](RackMultipart20230730-1-kkrz0p_html_4999e956a2405629.png)
...
![](RackMultipart20230730-1-kkrz0p_html_8ca1ed983d3e101d.png)
...
![](RackMultipart20230730-1-kkrz0p_html_1cceafac2f43b519.png)

_ **Resultado del aprendizaje en un entorno con obstáculos:** _

_ **Resultados del Q-learning con 1000 episodios de entrenamiento** _
...
![](RackMultipart20230730-1-kkrz0p_html_efd61fcd77d230de.png)
...
![](RackMultipart20230730-1-kkrz0p_html_eafda73177b90a2c.png)
...
![](RackMultipart20230730-1-kkrz0p_html_c9d374292fe4a8d.png)
...
![](RackMultipart20230730-1-kkrz0p_html_c32ded38dedfb560.png)
...
![](RackMultipart20230730-1-kkrz0p_html_9a00877ea50f6b71.png)

_ **Resultados del Deep Q-learning con 250 episodios de entrenamiento** _
...
![](RackMultipart20230730-1-kkrz0p_html_7ce9905ecf6f971b.png)

_ **Resultados en base al número de movimientos realizados durante el aprendizaje.** _
...
![](RackMultipart20230730-1-kkrz0p_html_7ebaf4058ac1b427.png)
...
![](RackMultipart20230730-1-kkrz0p_html_4842ca1f8849346a.png)
...
[https://es.wikipedia.org/wiki/Neurona\_de\_McCulloch-Pitts](https://es.wikipedia.org/wiki/Neurona_de_McCulloch-Pitts)
